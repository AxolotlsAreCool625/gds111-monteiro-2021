...
function animate()
{
	...
	dx = Math.abs(player.x - pearl.x);
	dy = Math.abs(player.y - pearl.y);
	...
	if(distance <= 300) {
		...
		pearl.ax = Math.cos(radians);		
		pearl.ay = Math.sin(radians);		

		if(pearl.x > player.x) {
			pearl.vx += pearl.ax * pearl.force;
		}
		else if (pearl.x < player.x) {
			pearl.vx += pearl.ax * -pearl.force;
		}

		if(pearl.y > player.y) {
			pearl.vy += pearl.ay * pearl.force;
		}
		else if (pearl.y < player.y) {
			pearl.vy += pearl.ay * -pearl.force;
		}

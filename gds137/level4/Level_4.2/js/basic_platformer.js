...
function animate()
{
	...
	*/
	...
	while(platform0.hitTestPoint({x:player.x - player.x/4, y:player.y + player.y/8}) && player.vy >=0)
	{
		player.y--;
		player.vy = 0;
		player.canJump = true;
	...
	while(platform1.hitTestPoint({x:player.x + player.x/8, y:player.y + player.y/8}) && player.vy >=0)
	{
		player.y--;
		player.vy = 0;
		player.canJump = true;

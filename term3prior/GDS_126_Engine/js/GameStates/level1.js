...
//The ground
var ground = new GameObject({width:canvas.width*10, x:canvas.width*5/2,height:64,y:canvas.height-32, color:"green"})
...
var rbg = new GameObject({x:level.x, y:level.y - 64, width:1024, height:512})
rbg.img.src=`images/Building_Background.png`
...
gameStates[`level1`] = function()
{
	...
	if(keys[` `] )
	{
		if(canShoot)
		{
			...
			bullets[currentBullet].vx = 6*wiz.dir;
	...
	for(let i=0; i<bullets.length; i++)
	{
		...
		bullets[i].move()
		if(bullets[i].x < 3220 && bullets[i].x > 0) bullets[i].play(function(){return}).drawSprite();
		bullets[i].angle+=8
